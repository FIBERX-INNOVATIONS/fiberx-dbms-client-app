...
class RegisteredAppSchemaMenuListConfig {
    public readonly name = "registered_app_schema_menu_list_config";
    ...
    public static getBulkActionMenuList (
        ...
        const can_member_bulk_delete    = member_authenticator.canMemberAccess("bulk_delete_registered_app_schema", LOCAL_STORAGE_FIELDS.MEMBER_PERMISSIONS_KEY)
        ...
        if(can_member_bulk_delete) {
            ...
            menu_list.push(
                this.buildMenuItem("BulkDeleteRegisteredAppSchemas", bulk_delete_menu_text, "", bulk_delete_menu_svg_icon, on_click)
            );
    ...
    public static getTableMenuList (
        ...
        const view_menu             = this.buildMenuItem(`ViewRegisteredAppSchema-${record_index}`, view_menu_text, "", view_menu_svg_icon, view_menu_on_click);
        ...
        const select_menu           = this.buildMenuItem(`SelectRegisteredAppSchema-${record_index}`, select_menu_text, "", select_menu_svg_icon, select_menu_on_click);

        const menu_list: NavLinkUIPropsInterface[] = [view_menu, select_menu];

        if(member_authenticator.canMemberAccess("update_app_schema", member_perm_key)) {
            const edit_menu_on_click  = (event: MouseEvent) => { return event_handler.handleOpenFormModal.bind(event_handler)(event, record);}
            const edit_menu           = this.buildMenuItem(`EditRegisteredAppSchema-${record_index}`, edit_menu_text, "", edit_menu_svg_icon, edit_menu_on_click);

            menu_list.push(edit_menu)
        }

        if(member_authenticator.canMemberAccess("delete_app_schema", member_perm_key)) {
            const delete_menu_on_click  = (event: MouseEvent) => { return event_handler.handleConfirmDelete.bind(event_handler)(event, record); }
            const delete_menu           = this.buildMenuItem(`DeleteRegisteredAppSchema-${record_index}`, delete_menu_text, "", delete_menu_svg_icon, delete_menu_on_click);
...
export default RegisteredAppSchemaMenuListConfig
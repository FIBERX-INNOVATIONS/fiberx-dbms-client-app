...
class RegisteredAppSchemaUIService extends BaseService {
    ...
    public async executeFetchRecord(schema_id: string | number): Promise<{s_state: boolean, s_msg: string, s_data?: Record<string, any>, logout?: boolean}> {
        try {
            const { status, msg, data: response_data } = await this.api_service.getRegisteredAppSchemaRecord(schema_id);

...
import RegisteredAppSchemaProfileView           from "@/modules/registered_app_schema_module/views/profile_view/registered_app_schema_profile_view.vue";
import RegisteredAppSchemaFormView              from "@/modules/registered_app_schema_module/views/form_view/registered_app_schema_form_view.vue";
...
class RegisteredAppSchemaListViewEventHandler extends BaseEventHandler {
    ...
    public async afterRecordSelected (updated_array: string[]) {
        ...
        const new_table_header_props    = BaseListViewPropsBuilder.getDataTableHeaderProps(this, content_field_key, RegisteredAppSchemaTableColumnConfig, order_by, order_direction, records.length, updated_array);
        const new_table_body_props      = BaseListViewPropsBuilder.getDataTableBodyProps(this, content_field_key, RegisteredAppSchemaTableColumnConfig, record_id_key, order_by, order_direction, records, updated_array);
    ...
    public async updateTableBodyProps (updated_array: Record<string, any>[]) {
        ...
        const new_table_body_props = BaseListViewPropsBuilder.getDataTableBodyProps(
            this, 
            content_field_key, 
            RegisteredAppSchemaTableColumnConfig, 
            record_id_key,
            order_by, 
            order_direction, 
            updated_array,
    ...
    public async handleOpenProfileModal (event: Event | InputEvent, record: Record<string, any>) {
        try {
            const content_data      = this.content_manager?.get("content_resource.registered_app_schema_view_ui.registered_app_schema_profile");
            const { title_text }    = content_data;
            const title_content     = title_text.replace("%", record?.name);
            const component         = markRaw(RegisteredAppSchemaProfileView);
    ...
    public async handleOpenFormModal (event: Event | InputEvent, record: Record<string, any> = {}) {
        try {
            const content_data = this.content_manager?.get("content_resource.registered_app_schema_view_ui.registered_app_schema_form");
            ...
            const component         = markRaw(RegisteredAppSchemaFormView);
            const component_props   = { record };
    ...
    public async handleOpenConfirmModal (
        ...
        try {
            ...
            const content_data = this.content_manager?.get(`content_resource.registered_app_schema_view_ui.${content_data_key}`);
    ...
    public async handleDeleteARecord (event: MouseEvent | InputEvent, record: Record<string, any> = {}) {
        ...
        try {
            ...
            const { s_state, s_msg, logout } = await this.controller.service?.executeDeleteRecordSchema(record_id);
    ...
    // Method to handle create a registered_app_schema instance on confirm btn click
    ...
    // Method to handle destroy a registered_app_schema instance on confirm btn click

...
import ConfirmActionUI                  from "@ui/version_2/components/ConfirmActionUI/confirm_action_ui.vue";
...
class BaseListViewEventHandler extends BaseEventHandler {
    ...
    constructor(
        ...
        this.table_column_config            = table_column_config;
        this.profile_view_component         = profile_view_component;
        this.form_view_component            = form_view_component;
    ...
    public async handleFormActionBtnClick (event: MouseEvent) { this.handleOpenFormModal(event, {}); }
    ...
    // Method to handle toggling ellipsis dropdown
    public async handleBulkDeleteActionClick (event: MouseEvent) { this.logger.log(`Bulk Delete button clicked`); }
    ...
    public async handleOpenConfirmModal (
        ...
        try {
            ...
            const content_data          = this.content_manager?.get(`content_resource.${content_field_key}.${content_data_key}`);
    ...
    public async handleDeleteARecord (event: MouseEvent | InputEvent, record: Record<string, any> = {}) {
        ...
        try {
            ...
            const { s_state, s_msg, logout } = await this.controller.service?.executeDeleteRecord(record_id);
...
export default BaseListViewEventHandler;